...
export class ChangePasswordService {
  ...
  async changePassword(changePasswordDto: ChangePasswordInput) {
    ...
    // find local account if have a password
    const authAccount = await this.usersService.findAuthAccount(
      user.id,
      'local',
    );

    if (!authAccount?.hashedPassword) {
      await bcrypt.compare(
        'dummy_password',
        '$2b$10$invalidsaltinvalidsaltinv',
      ); // dummy compare
      throw new UnauthorizedException('Authentication method not found');
    }
